      ...
      <?php foreach ($equipo['seisPokemons'] as $pokemon): ?>
        ...
        <?php if ($pokemon['id_pokemon'] != 0) { ?>
          ...
          <div onclick="confirmarEliminacion(<?= $pokemon['id_equipopokemon']; ?>)">
            <img src="<?= $pokemon['art']; ?>" id="<?= $pokemon['id_equipopokemon']; ?>" name="<?= $pokemon['id_equipopokemon']; ?>"
              class="w-48 h-48 mx-auto" id="imagenLinkPokemon"><!-- .end img tag -->
          </div>
...
<div id="renameModal" class="fixed inset-0 flex items-center justify-center z-50 hidden">
  <div class="bg-white p-6 rounded-md shadow-md w-80">
    ...
    <form id="teamRenameForm">
      ...
      <input type="text" placeholder="Nuevo nombre del equipo" class="border p-2 w-full mb-4" id="nuevoNombreEquipo"
        name="nuevoNombre">

      <button id="confirmRenameButton" class="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-md"
        type="submit">Confirm
      </button>
...
<div id="notification-container" class="fixed top-0 right-0 m-4">
  <div id="notification"
    class="flex items-center p-4 mb-4 text-sm text-green-800 border border-green-300 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400 dark:border-green-800 hidden"
    role="alert">
    ...
    <div>
      <span class="font-medium" id="notification-title">Success! Hooray!</span> <span
        id="notification-message">Everything went according to keikaku.</span>
    </div>
  </div>
</div>
