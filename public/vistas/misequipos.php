...
<?php foreach ($equipos as $equipo) : ?>
  <div class="equipo">
    <h3><?= $equipo['nombre'] ?>
      <a href="#renameModal" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md" data-id="<?= $equipo['id'] ?>">Rename</a>
    </h3>
    ...
    <?php foreach ($equipo['seisPokemons'] as $pokemon) : ?>
      ...
      <?php if ($pokemon['id_pokemon'] != 0)  { ?>
        ...
        <div onclick="confirmarEliminacion(<?=$pokemon['id_equipopokemon'];?>)">
          <img src="<?=$pokemon['art'];?>" id="<?=$pokemon['id_equipopokemon'];?>" name="<?=$pokemon['id_equipopokemon'];?>" class="w-48 h-48 mx-auto" id="imagenLinkPokemon"><!-- .end img tag -->
        </div>
...
<div id="modal-conf-eliminacion" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
  <div class="relative p-4 w-full max-w-md max-h-full">
    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
      <button id="botonXcerrarModalConfEliminacion" type="button" class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="popup-modal">
...
<div id="renameModal" class="fixed inset-0 flex items-center justify-center z-50 hidden">
  <div class="bg-white p-6 rounded-md shadow-md w-80">
    ...
    <form id="teamRenameForm">
      ...
      <input type="text" placeholder="Nuevo nombre del equipo" class="border p-2 w-full mb-4" id="nuevoNombreEquipo" name="nuevoNombre">

      <button id="confirmRenameButton" class="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-md" type="submit">Confirm
      </button>
