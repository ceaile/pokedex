...
<div class="pokemon-container" style="opacity: 0;">
  ...
  <?php foreach ($equipos as $equipo): ?>
    <div class="equipo" style="margin-bottom: 15px; overflow-x: auto; white-space: nowrap;">
      <!-- Estilo añadido para desplazamiento horizontal -->
      <div style="text-align: center;"> <!-- Contenedor centrado para el título y el botón -->
        <div
          style="background-color: white; border-radius: 5px; display: inline-block; padding: 10px; margin-bottom: 10px;">
          <!-- Estilo para el contenedor del título y botón -->
          <h3
            style="margin: 0; background-color:white; display: inline-flex; align-items: center; justify-content: center;">
            <?= htmlspecialchars($equipo['nombre']) ?>
          </h3> <!-- Título -->
          <a href="#renameModal" class="bg-blue-500 hover:bg-blue-700 text-white px-4 py-2 rounded-lg"
        ...
        <?php foreach ($equipo['seisPokemons'] as $pokemon): ?>
          <?php if ($pokemon['id_pokemon'] != 0): ?>
            <div class="pokemon-img-container"
              ...
              style="display: inline-block; margin-right: 10px; padding: 2px; background-color: white; border-radius: 5px; position: relative;">
              <img src="<?= htmlspecialchars($pokemon['art']) ?>" id="<?= htmlspecialchars($pokemon['id_equipopokemon']) ?>"
                name="<?= htmlspecialchars($pokemon['id_equipopokemon']) ?>" class="w-48 h-48" style="display: block;">
            </div>
          <?php endif; ?>
        <?php endforeach; ?>
      </div>
        ...
        <?php foreach ($equipo['seisPokemons'] as $pokemon): ?>
          <?php if ($pokemon['id_pokemon'] != 0): ?>
            <a href="card?id_pokemon=<?= htmlspecialchars($pokemon['id_pokemon']) ?>"
              class="view-button-mobile bg-yellow-500 hover:bg-yellow-700 text-white px-2 py-1 rounded-lg mt-2 block text-center"
...
<style>
  .pokemon-img-container {
    ...
    padding: 1px;
    /* Reducido el espacio entre la imagen y el borde blanco */
